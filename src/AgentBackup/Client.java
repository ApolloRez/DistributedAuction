...
public class Client {
    ...
    private BlockingQueue<Boolean> regCheck = new LinkedBlockingDeque<>();
    ...
    public Client(String address, int port){
        try{
            ...
            bankSocket.connect( new InetSocketAddress(address, port),2000);
    ...
    public Boolean getOneRegCheck(){
        try {
            return regCheck.take();
        }catch (InterruptedException e){
            e.printStackTrace();
            return false;
        }
    ...
    public boolean isRegistered(AuctionProxy auction){
        for(AuctionProxy temp: activeAuctions){
            if(temp.equals(auction)){
                return true;
            }
        }
        return  false;
